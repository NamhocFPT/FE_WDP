...
import { BookOpen, GraduationCap, ChevronRight, BarChart3, Clock } from "lucide-react";

export default function StudentGradeOverview() {
    ...
    useEffect(() => {
        const fetchOverview = async () => {
            try {
                ...
                // Dựa trên cấu trúc API: { status: "success", data: [...] }
                            ...
                            <Card key={g.class_id} className="group hover:shadow-xl transition-all duration-300 border-none shadow-sm ring-1 ring-slate-200">
                                <CardContent className="p-0">
                                    <div className="p-5 border-b border-slate-100">
                                        <div className="flex justify-between items-start mb-3">
                                            ...
                                            <Badge tone={g.course_total >= 5 ? "green" : g.course_total === null ? "slate" : "amber"}>
                                                {g.course_total !== null ? `TB: ${g.course_total.toFixed(2)}` : "Chưa có điểm"}
                                            </Badge>
                                        </div>
                                    ...
                                    <div className="p-5 bg-slate-50/50 space-y-3">
                                        ...
                                        <Button 
                                            variant="outline" 
                                            className="w-full mt-2 group/btn border-slate-300 hover:border-blue-600 hover:text-blue-600 transition-all"
