...
export default function MaterialsManagement() {
  ...
  const fetchClasses = async () => {
    try {
      ...
      const merged = [
        ...
        ...(Array.isArray(myClassesRes.data?.data)
      ...
      const uniqueClasses = merged.filter(
        (cls, index, arr) =>
          index === arr.findIndex((item) => item.class_id === cls.class_id),
      );
    ...
    if (selectedClassId) {
      ...
      if (classes.length > 0) {
        const cls = classes.find(
          (c) => String(c.class_id) === String(selectedClassId),
        );
        setIsUpcomingClass(
          !!cls && String(cls.status).toLowerCase() === "upcoming",
        );
      }
    } else {
      setGeneral([]);
  ...
  }, [selectedClassId, classes, fetchMaterials]);
    ...
    return (
      <div
        ...
        className={cn(
          ...
          isUpcomingClass && "pointer-events-none",
        )}
      >
  ...
  return (
    <div className="pb-10">
      <PageHeader
        ...
        right={[
          <Button
            ...
            className="gap-2"
            disabled={!selectedClassId || isUpcomingClass}
            title={isUpcomingClass ? "Lớp học chưa bắt đầu" : ""}
          >
            <Upload size={16} />
            Tải lên
          </Button>,
      ...
      {isUpcomingClass && (
        <div className="mb-6 flex items-start gap-2 rounded-xl border border-blue-200 bg-blue-50 p-4 text-sm text-blue-800 shadow-sm">
          <span className="pt-0.5 text-blue-500">ℹ️</span>
          <div>
            <div className="font-bold">Lớp học chưa bắt đầu (Sắp tới)</div>
            <p className="mt-1 opacity-90">
              Bạn không thể tải lên hoặc chỉnh sửa tài liệu cho lớp học này cho
              đến khi lớp học bắt đầu.
            </p>
          </div>
        </div>
      )}
                ...
                <section key={session.id}>
                  ...
                  >
                    <div className="flex items-center gap-2 font-semibold text-slate-800">
                      ...
                      BUỔI {session.index} — {formatDate(session.start_time)} —{" "}
          ...
          <div className="space-y-1 text-sm">
            ...
            >
              ...
              {bySession.map((sessionGroup) => (
                ...
                >
                  {`Buổi ${sessionGroup.session.index} - ${formatDate(
                    sessionGroup.session.start_time,
