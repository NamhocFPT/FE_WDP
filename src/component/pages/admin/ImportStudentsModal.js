...
export default function ImportStudentsModal({ isOpen, onClose, classId, onSuccess, onDownloadTemplate }) {
    ...
    const parseEmailsFromFile = (uploadedFile) => {
        ...
        reader.onload = async (e) => {
            try {
                ...
                const res = await adminApi.validateStudentImport(classId, rows);
                if (res.data.success) {
                    ...
                    if (invalidCount > 0) {
                        ...
                        setActiveTab("valid");
                        toast.success(`Tìm thấy ${validCount} email hợp lệ.`);
                    }
                }
            } catch (error) {
        ...
        reader.onerror = () => { toast.error("Không thể đọc được file."); setFile(null); }
    ...
    const handleFileChange = (e) => { const f = e.target.files[0]; if (f) parseEmailsFromFile(f); };
    const handleDrag = (e) => { e.preventDefault(); e.stopPropagation(); setDragActive(e.type === "dragenter" || e.type === "dragover"); };
    const handleDrop = (e) => { e.preventDefault(); e.stopPropagation(); setDragActive(false); if (e.dataTransfer.files?.[0]) parseEmailsFromFile(e.dataTransfer.files[0]); };

    const handleConfirmImport = async () => {
        if (!previewData || previewData.validRows.length === 0) return;
        setLoading(true);
        try {
            await adminApi.confirmStudentImport(classId, previewData.validRows);
            toast.success("Import học sinh thành công!");
            if (onSuccess) onSuccess();
            onClose();
        } catch (error) {
            toast.error(error.response?.data?.message || "Có lỗi xảy ra khi xác nhận import");
            ...
            <div className="overflow-x-auto border rounded-xl">
                <table className="min-w-full divide-y divide-slate-200">
                    ...
                    <tbody className="divide-y divide-slate-200 bg-white">
                        {rows.map((r, i) => (
                            <tr key={i} className={`hover:bg-slate-50 text-xs text-slate-700 ${type === "invalid" ? "bg-red-50/30" : ""}`}>
                                <td className={`px-3 py-2 ${type === "invalid" ? "text-red-700 font-bold" : "text-slate-500"}`}>{r.rowNumber}</td>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
            <div className="bg-white rounded-2xl w-full max-w-4xl shadow-xl overflow-hidden flex flex-col max-h-[85vh]">
                <div className="flex justify-between items-start p-6 pb-4 shrink-0">
                    <div>
                        <h3 className="text-xl font-bold text-slate-900">Nhập học sinh từ file</h3>
                        <p className="text-sm text-slate-500 mt-1">
                            Tải lên tệp Excel chứa danh sách email học sinh
                            {onDownloadTemplate && <button onClick={onDownloadTemplate} className="text-blue-600 hover:underline font-medium ml-1">Tải xuống file mẫu</button>}
                        </p>
                    </div>
                    ...
                    {!previewData && (
                        ...
                        >
                            <div className="text-center">
                                ...
                                <p className="text-xs text-slate-500 mt-1">Hỗ trợ đọc Cột: Họ tên, Email</p>
                            </div>
                        </div>
                    )}
                ...
                <div className="p-6 pt-4 flex justify-end gap-3 shrink-0 border-t border-slate-100">
                    ...
                    {previewData && (
                        ...
                        >
                            ...
                            Xác nhận Import ({validCount} học sinh)
                        </button>
                    )}
                </div>
            </div>
